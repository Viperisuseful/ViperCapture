...
ViperCapture remains beta until every gate below has dated, reviewable evidence.
Passing unit tests or one successful package build is necessary but insufficient.
...
The operational and benchmark workflows provide the repeatable starting point:
...
Those workflow defaults are regression gates, not the one-hour v1 evidence.
