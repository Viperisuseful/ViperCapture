...
from vipercapture.schedules import (
    ScheduleCreate,
    ScheduleService,
    ScheduleStore,
    ScheduleUpdate,
...
class ScheduleTests(unittest.IsolatedAsyncioTestCase):
    async def test_bundled_store_refuses_windows(self):
        with tempfile.TemporaryDirectory() as directory:
            store = ScheduleStore(Path(directory) / "vipercapture.schedules.sqlite3")
            with (
                patch("vipercapture.schedules.os.name", "nt"),
    ...
    async def test_sqlite_operations_run_off_the_event_loop(self):
        ...
        with patch(
            "vipercapture.schedules.asyncio.to_thread",
    ...
    async def test_claimed_occurrence_survives_store_restart(self):
        ...
        self.store = ScheduleStore(
            Path(self.temporary.name) / "vipercapture.schedules.sqlite3"
        )
        await self.store.start()
