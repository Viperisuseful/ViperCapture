...
import vipercapture.async_jobs as async_jobs
from vipercapture.async_job_providers import LocalArtifactStore, SQLiteJobStore
from vipercapture.async_jobs import (
    Artifact,
    ArtifactStoreConfig,
    AsyncJobService,
...
class AsyncJobServiceTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_bulk_claim_outlives_retained_item_metadata(self):
        ...
        item_key = "@bulk-header-item:7:release:0"
        ...
        await service.submit(
            ...
            idempotency_key="@bulk-header-item:10:other:2026:0",
        )
        await self.store.claim_bulk_idempotency(
            prefix_key,
            b"new",
    ...
    async def test_result_expiring_during_fetch_is_not_returned(self):
        ...
        try:
            ...
            with patch("vipercapture.async_jobs.datetime", wraps=datetime) as clock:
    ...
    async def test_local_artifact_is_synced_before_put_returns(self):
        await self.artifacts.start()
        with patch(
            "vipercapture.async_job_providers.os.fsync",
            wraps=os.fsync,
        ) as sync:
    ...
    async def test_local_artifact_fifo_candidates_open_nonblocking(self):
        ...
        with patch(
            "vipercapture.async_job_providers.os.open",
    ...
    def test_artifact_directory_sync_is_skipped_on_windows(self):
        with (
            patch("vipercapture.async_job_providers.os.name", "nt"),
            patch("vipercapture.async_job_providers.os.open") as open_file,
        ):
            self.artifacts._sync_directory()
        open_file.assert_not_called()

    async def test_bundled_stores_refuse_unprotected_windows_data(self):
        with patch("vipercapture.async_job_providers.os.name", "nt"):
    ...
    async def test_sqlite_database_and_wal_entries_are_synced_on_start(self):
        from vipercapture.async_job_providers import _sync_directory
        ...
        with patch(
            "vipercapture.async_job_providers._sync_directory",
...
class ProviderLoadingTests(unittest.TestCase):
    def test_cache_defaults_to_data_directory_without_async_jobs(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [sys.executable, "-c", "import vipercapture.main as main; print(main.CACHE_DIRECTORY)"],
    ...
    def test_generated_local_key_is_private_and_restart_stable(self):
        with tempfile.TemporaryDirectory() as directory, patch.dict(
            ...
            root = Path(directory)
            with (
                patch("vipercapture.async_jobs.os.fsync", wraps=os.fsync) as sync,
                patch("vipercapture.async_jobs.os.link", wraps=os.link) as link,
            ):
    ...
    def test_new_key_data_directory_entry_is_synced(self):
        with tempfile.TemporaryDirectory() as directory, patch.dict(
            ...
            with patch(
                "vipercapture.async_jobs._sync_directory",
                wraps=async_jobs._sync_directory,
            ) as sync:
    ...
    def test_fifo_key_candidate_is_opened_nonblocking(self):
        with tempfile.TemporaryDirectory() as directory, patch.dict(
            ...
            with patch("vipercapture.async_jobs.os.open", side_effect=checked_open):
    ...
    def test_key_directory_sync_is_skipped_on_windows(self):
        ...
        with (
            ...
            patch("vipercapture.async_jobs.os.open") as open_file,
        ):
            from vipercapture.async_jobs import _sync_directory
...
class AsyncJobRouteTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_async_render_timing_includes_capture_slot_wait(self):
        ...
        try:
            with (
                patch(
                    "vipercapture.main.time.perf_counter",
                    side_effect=[10.0, 10.0, 10.25, 10.25, 10.35, 10.35],
                )
                as clock,
                patch("vipercapture.main.RenderEngine") as engine_class,
    ...
    async def test_provider_close_error_does_not_skip_browser_cleanup(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            with (
                ...
                patch("vipercapture.main.ASYNC_JOB_SETTINGS", _settings(Path("."))),
                patch("vipercapture.main.CACHE_DIRECTORY", Path(directory) / "cache"),
                patch("vipercapture.main.load_providers", return_value=(object(), object())),
                patch("vipercapture.main.AsyncJobService", return_value=service),
            ):
    ...
    async def test_start_failure_bounds_partially_started_provider_cleanup(self):
        ...
        with (
            patch("vipercapture.async_jobs.asyncio.wait", wraps=asyncio.wait) as wait,
